...
- **Inactive workspace windows are parked again after a successful resume.** Resume still applies only the active workspace, then catches up inactive tiled and floating windows. Pause continues to leave native windows in place after a minimized-flag resync, and a failed resume rolls back to paused without moving those windows.
